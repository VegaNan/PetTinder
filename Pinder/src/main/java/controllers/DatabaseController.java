...
public class DatabaseController {
	...
	public Animal[] createAnimalObjects(String dbString) {
		...
		for (int i = 0; i < animalNum; i++) {
			...
			String location = jo.getJSONObject("contact").getJSONObject("address").getString("city")
					+ jo.getJSONObject("contact").getJSONObject("address").getString("state")
					+ jo.getJSONObject("contact").getJSONObject("address").getString("postcode");
			double distance = Double.parseDouble(jo.get("distance").toString());
			boolean spayedNeutered = Boolean
					.parseBoolean(jo.getJSONObject("attributes").get("spayed_neutered").toString());
			...
			animals[i] = new Animal(id, organizationId, type, breed, size, gender, age, status, name, organization,
					goodWithChildren, goodWithDogs, goodWithCats, location, distance, spayedNeutered, houseTrained,
	...
	public Organization[] createOrganizationObjects(String dbString) {
		...
		for (int i = 0; i < dbLength; i++) {
			...
			orgArr[i] = new Organization(organizationId, name, location, state, country, contactEmail, contactPhone,
					zipcode, websiteUrl);
		}
	...
	public void storeUser(User user) {
		Document document = new Document("firstName", user.getFirstName()).append("lastName", user.getLastName())
				.append("password", user.getPassword()).append("location", user.getLocation())
				.append("email", user.getEmail()).append("pref", user.getAnimalPref());
	...
	public User getUser(String email, String password) {
		...
		for (Document doc : cursor) {
			...
			if (password.equals(correctPassword)) {
				user = new User(doc.getString("firstName"), doc.getString("lastName"), doc.getString("password"),
						doc.getString("email"), doc.getString("location"));
			}
		}
		return user;
	}
